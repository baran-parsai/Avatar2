 ...
 * 
 * Version History
 * 4.0 - made to be consistent with ROS2 modifications
 * 3.1 - changed methods to start with an upper case letter to be more consistent with c#
 * style.
 * 3.0 - modification from hand crafted version 2.0
 * 
 * @author Michael Jenkin and Walled Khan. Based on Version 3.1 by Robert Codd-Downey and Andrew Speers
 * @version 4.0
 */
...
namespace ROSBridgeLib
{
	public class ROSBridgeWebSocketConnection
	{
		...
		private void Run()
		{
			...
			_ws.Connect();
			Debug.Log("Connected " + _host + ":" + _port);
			...
			if ((s != null) && !s.Equals(""))
			{
				...
				if ("publish".Equals(op))
				{
					...
					foreach (Type p in _subscribers)
					{
						...
						{
							// Debug.Log("Parsing it");
							ROSBridgeMsg msg = ParseMessage(p, node["msg"]);
							// Debug.Log("It parsed as " + msg);
							...
							lock (_queueLock)
							{
								...
								for (int i = 0; i < _taskQ.Count; i++)
								{
									if (_taskQ[i].getTopic().Equals(topic))
									{
										...
										Debug.Log("Adding render task (dup)");
										found = true;
										break;
									}
								}
								...
								{
									_taskQ.Add(newTask);
									Debug.Log("adding render task (no dupe)");
								}
							}
				...
				else if ("service_response".Equals(op))
				{
					...
					Debug.Log("Must write code here for other messages " + op);
			}
			else
